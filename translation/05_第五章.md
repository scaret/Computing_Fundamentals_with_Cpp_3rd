...
和大多数类一样，`string`类的成员函数中也有负责修改`string`对象状态的（譬如`insert`、`replace`、`erase`）和只返回对象状态的（譬如`length`、`find`、`substr`）。另外，`string`类中还有个允许访问其元素或个别字符的操作：`[]`、`front`和`back`。最后，还有一组作用于`string`对象本身的操作，譬如`+`、`[]`、`<<`和`>>`。

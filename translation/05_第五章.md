...
需要注意的是，C++社区中指称类属函数时使用的是*成员函数（member function）* 这个术语[^1]，而在指称存储对象状态的变量时则使用的术语是*数据成员（data member）*。在其他编程语言中，人们通常使用*方法（method）*这个术语来指称前者，而不是*成员函数*。后者也一样，人们通常使用的术语是*实例变量（instance variable）*而不是*数据成员*。本教材将采用C++的术语风格。

[^1]: 译者注：此处原文为data member，译者认为是作者笔误，故修正于此，特做说明。
...
> string *identifier*(*string-literal*);<br>
...
在这一节中，我们要来介绍另一个非标准类。我们在以后的章节中还会偶尔用到这个类，它有助于我们更深入地去思考对象的概念，这对提高解决问题的技能还是很有帮助的。
...
前两个实参指定的是`Grid`对象中矩阵的行数和列数，接下来两个实参指定的移动器（mover）最初所在的行数和列数，最后一个实参指定的是移动器的初始方向，这里有`north`、`south`、`east`、`west`四个方向。
...
我们也可能会向`Grid`对象发送许多“非法”的消息。例如，我们在向其发送`move`消息时会让移动器撞上障碍物（`#`）或者直接将其移出`Grid`的地图边界，以及所有不正确的消息（譬如实际要移动的是四行而不是三行等）。
...
这种尴尬情景要利用前置条件的概念来避免。函数的“前置条件”是指该函数只能在某个假设成立时才能执行函数调用或发送消息的动作。例如，`void move(int spaces)`这个操作的前置条件是移动器的移动路径上不能撞上障碍物或网格边界。同样地，`Grid::pickUp()`的假设则是移动器所在的位置必须要有东西可以被拿起来。
...
请编写一个C++程序，目的是获取五个单词中的隐藏信息。我们要求该程序要能从五个输入字符串中各取一个字符，然后将它们拼成一个新的单词。该程序除了能按照下列过程运行两次之外，还能再补充一次会话：
...
请编写一段代码，以`main`函数的形式构建一个13乘7的`Grid`对象，然后指示移动器“绘制”出一个字母I，其效果如下图所示（移动器可以留在I旁边的任何位置）：
...
请编写一个函数`void jumpOneHurdle(Grid & g)`，以指示移动器跳过一个“障碍物”（这里用`#`来表示）。要求在`main`函数中要调用该函数五次，并在每次调用`jumpOneHurdle`之后都要显示一次移动器的当前状态。
...
下面是第一次向`Grid`对象发送`display`消息时，该对象的状态：
...
下面是第六次发送`display`消息时，移动器跳过第五个障碍物之后的情况：
...
请编写一个函数`void climbStair(Grid & g)`，以指示移动器如何爬行一步，并通过对该函数足够次数的调用让移动器爬到指定楼梯的顶部。在这里，我们需要发送六次`block`消息来模拟下面这样的楼梯：
...
请编写一个C++程序，使其能以下`main`函数测试驱动，通过调用其指定的十个自由函数产生我们下面所预计的输出。
...
大家知道，星际航行所用的燃料通常是物质与反物质的混合。现在，请按照这个想法编写一个`antiMatter`函数，在实参字符串所指定的事物或思想名词之前加一个“Anti-”前缀，注意，不要忘了这里的连接符。
...
请编写一个`removeEnds`函数，令其返回实参字符串去除两端字母之后的子字符串。该函数的前置条件是其参数字符串中至少要有两个字符。
...
请编写一个`tripleUp`函数，令其将实参字符串的内容分别以1)、2)、3)为前缀重复三次，然后将其连接单一字符串并返回。该函数的前置条件是`str.length()`大于等于1。
...
该函数所要处理的字符串必须长度在2以上，然后它会将第二实参字符串插入到第一实参字符串的中间，构成一个新字符串并返回。如果第一实参字符串的长度为奇数，则允许后半个字符串的长度大于前半个字符串。
...
请编写一个`halfAndHalf`函数，令其将实参字符串的前半部分与后半部分进行首尾互换位置，构成一个新字符串并返回。如果实参的字符数量为奇数，则允许分割后的后半部分比前半部分多一个字符。该函数的前置条件为`str.length()`大于等于2。
...
请编写一个`nameRearranged`函数，使其能将实参字符串所指定的姓名格式从`lastName`+", "+`firstName`+`initial`转换成`firstName`+`initial`+". "+`lastName`的格式，并以字符串的形式返回。
...
请编写一个`middleThree`函数，令其返回实参字符串的中间三个字符（当然，该字符串的字符数必须在3以上）。如果实参字符串长度为偶数，则从右侧开始计数。该函数的前置条件为`str.length()`大于等于3。
...
请编写一个`reverse7Chars`函数，令其将实参字符串的内容反转并返回。该函数的前置条件是函数实参`str`要由七个字符组成。
...
请编写一个`remove3`函数，该函数会修改实参字符串`str`，将其中前三次出现的`sub`参数所指定的子串移除。该函数的前置条件是`sub`所指定的子串至少要在`str`中出现三次。
...
请编写一个`replace`函数，该函数会修改实参字符串`str`，将其中前三次出现的`oldC`所指定的字符替换成`newC`所指定的字符。该函数的前置条件是`oldC`参数所指定的字符至少要在`str`中出现三次。

...
You have now experienced generic vector objects as a way to store a collection of related elements of a specifi c type.

## 本章提要

This chapter introduces another collection class named Set to provide a review of vector processing, class defi nitions, and member function implementations. This chapter also introduces the C++ template mechanism that allows a collection to store a specific type of object. After studying this chapter you will be able to
...
The Set collection class used in this chapter provides a review of class definitions and member function implementations. This time, however, member functions will employ vectorprocessing algorithms. You will also learn how collection classes can be made to store only a specific type. Th e type can be passed as an argument to the class when constructed.
...
Th ese messages will not compile since the argument is not the proper type (which is a good thing, actually).
